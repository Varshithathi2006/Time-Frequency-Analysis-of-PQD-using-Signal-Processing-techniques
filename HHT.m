...
all_imfs = cell(disturbance_classes, 1);
for i = 1:disturbance_classes
    signal = signals{i};
    all_imfs{i} = emd(signal);
end

% Perform Hilbert Transform on each IMF to get Hilbert Spectrum
hs = cell(disturbance_classes, 1);
f = cell(disturbance_classes, 1);
t = cell(disturbance_classes, 1);
for i = 1:disturbance_classes
    imfs = all_imfs{i};
    [hs{i}, f{i}, t{i}] = hht(imfs, Fs);
end
