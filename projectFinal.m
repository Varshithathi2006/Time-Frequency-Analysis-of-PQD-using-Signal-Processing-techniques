...
fileID = fopen('disturbance_values.csv', 'w');
...
% Define parameters for the SST
waveletType = 'amor';   % Type of wavelet (Morlet wavelet in this case)
voicesPerOctave = 48;   % Number of voices per octave
Fs = 1000;              % Sampling frequency (Hz)

% Initialize cell arrays to store results
cwtResults = cell(disturbance_classes, 1);
sstResults = cell(disturbance_classes, 1);
...
for i = 1:disturbance_classes
    ...
    cfs = cwt(signal, 'amor', Fs, 'VoicesPerOctave', voicesPerOctave);
    cwtResults{i} = cfs;
    
   % Perform Fourier Synchrosqueezed Transform (FSST)
    [wt, f] = fsst(signal, Fs);
    fsstResults{i} = wt;
    frequencies{i} = f;
end
...
for i = 1:disturbance_classes
    ...
    subplot(3, 1, 2);
    tfr = abs(cfs);
    imagesc(t, linspace(0, Fs/2, size(tfr, 1)), tfr);
    axis xy;
    ...
    imagesc(t, fs, sfs);
    axis xy;
    title('Fourier Synchrosqueezed Transform');
    xlabel('Time (s)');
    ylabel('Frequency (Hz)');
end
